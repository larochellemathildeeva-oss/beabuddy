  ...
  return (
    <section data-guide="compare-pins" className="card-soft p-3.5">
      <div className="flex items-baseline justify-between">
        ...
        <button onClick={() => setOpen((v) => !v)} className="text-[12.5px] text-primary">
      ...
      {!open ? (
        ...
        <div className="mt-3 space-y-3">
          ...
          <button
            ...
            className="w-full rounded-xl bg-primary px-4 py-2.5 text-[15px] font-medium text-primary-foreground disabled:opacity-50"
          >
            {busy
          ...
          {busy && (
            <p className="text-[13.5px] text-muted-foreground">{beaLine("choose.working").body}</p>
          )}
          ...
          {result && (
            <div className="rise space-y-2 rounded-2xl border border-border bg-elevated p-3">
              <p className="text-[12.5px] font-semibold uppercase tracking-wider text-primary">
              ...
              {result.reasoningText && (
                <div>
                  ...
                  >
                    ...
                    <span className={`transition-transform ${showThoughts ? "rotate-90" : ""}`}>
                      ▸
                    </span>
                  </button>
                  {showThoughts && (
              ...
              <button
                ...
                className="w-full rounded-xl border border-primary px-4 py-2 text-[15px] font-semibold text-primary"
              >
        ...
        >
          <div className="flex items-start gap-2 border-b border-border p-4">
            <div className="min-w-0 flex-1">
              ...
              <h3 className="font-display text-[23px] leading-tight">{result.headline}</h3>
            </div>
            <button
          ...
          <div className="flex-1 overflow-auto p-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[16.5px] font-semibold">{p.name}</p>
                      {winner && (
            ...
            <p className="mt-4 text-[13.5px] text-muted-foreground">{result.tip}</p>
            <p className="mt-1 text-[12.5px] text-muted-foreground">
              Swipe across to see each place.
            </p>
          </div>
        </div>
      )}
...
function MeasuredFacts({ result }: { result: ComparisonResult }) {
  ...
  if (
    !facts.pairs.length &&
    !facts.places.some((p) => p.daysSinceSaved != null || p.recommendedBy)
  ) {
    return null;
  }
  return (
