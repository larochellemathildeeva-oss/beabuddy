    ...
    <div>
      ...
      {showNew && allowCreate && (
        <div className="mb-3 space-y-2 rounded-xl bg-elevated p-3">
          ...
          <p className="text-[12px] text-muted-foreground">
            Pick a starter to fill the pack, leave it blank and add your own, or let Béa read photos
            or a file (up to 4 at once).
          </p>
          <input
            ref={cameraRef}
            type="file"
          ...
          {importPreview && (
            <div className="space-y-2 rounded-xl border border-border bg-elevated p-3">
              {importPreview.summary && (
                <p className="text-[13px] text-muted-foreground">{importPreview.summary}</p>
              )}
              {groupPackItems(
                ...
                <div key={group.section ?? "__none"}>
                  ...
                  <ul className="space-y-0.5">
                    {group.items.map((item, i) => (
                      <li key={`${item.label}-${i}`} className="text-[14.5px]">
                        {item.label}
          ...
          <button
            disabled={importBusy || (!newPack.trim() && !starter && !importPreview?.items.length)}
            onClick={async () => {
              const fromImport = Boolean(importPreview?.items.length);
              const name = newPack.trim() || importPreview?.name || starter.replace(/^\S+\s/, "");
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-border px-3 py-2 text-left text-[13px] font-semibold"
      >
