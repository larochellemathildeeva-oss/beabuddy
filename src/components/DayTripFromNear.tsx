...
export function DayTripFromNear({
  ...
  return (
    <div data-guide="day-trip" className="card-soft space-y-3 p-4">
      ...
      <div>
        <p className="text-[13px] font-medium">Pace for this day</p>
      ...
      {tags.length > 0 && (
        <div>
          <p className="text-[13px] font-medium">Lean into today</p>
      ...
      {draft && (
        <div className="space-y-2 rounded-xl border border-border bg-elevated p-3">
          <p className="text-[14.5px] font-semibold">{draft.title}</p>
          {draft.summary && <p className="text-[13px] text-muted-foreground">{draft.summary}</p>}
          <ol className="space-y-2">
            {draft.items.map((item, i) => (
              <li key={`${item.title}-${i}`} className="text-[14.5px]">
