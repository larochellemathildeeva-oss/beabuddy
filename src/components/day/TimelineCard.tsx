...
import { Car, ChevronDown, ChevronUp, Footprints, MapPinPlus, Plus } from "lucide-react";
      ...
      >
        ...
        >
          <div className="flex min-w-0 gap-3">
            {/* Time reads down the page as a column, so a day can be
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1">
            {!editing && (
              <button
                ...
                aria-pressed={done}
                className="min-h-8 text-[12px] font-semibold text-nexttime underline underline-offset-2"
              >
            ...
            {canKeep && (
              <button
                ...
                disabled={kept}
                onClick={keep}
                className="min-h-8 text-[12px] text-muted-foreground underline disabled:no-underline disabled:opacity-60"
              >
 ...
 *
 * From the prototype's transit connector: a line down the timeline and one
 * quiet pill saying how far and how long. The turn-by-turn steps open from
 * it, because they are only worth the space when you are about to walk
 * them. Hidden by the "Walk times" switch in Customize.
 */
  ...
  return (
    <li className="list-none">
      <div className="flex items-stretch gap-3 pl-6">
        <span aria-hidden className="w-px shrink-0 border-l-2 border-dashed border-border" />
        <div className="min-w-0 flex-1 py-0.5">
          <button
            type="button"
            onClick={() => setOpen(!open)}
            aria-expanded={open}
            className="inline-flex min-h-9 max-w-full items-center gap-2 rounded-full border border-border/70 bg-card px-3 text-left text-[12.5px] font-medium text-muted-foreground"
          >
...
/** Between two cards: add a stop there, at a time halfway between them. */
export function AddBetween({ onAdd }: { onAdd: () => void }) {
  return (
    <li className="list-none">
      <div className="flex items-center gap-3 pl-6">
        <span aria-hidden className="h-6 w-px shrink-0 border-l-2 border-dashed border-border" />
        <button
          type="button"
          onClick={onAdd}
          className="inline-flex min-h-8 items-center gap-1.5 rounded-full border border-dashed border-primary/40 bg-card px-3 text-[12px] font-semibold text-primary"
        >
