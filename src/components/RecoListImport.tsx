    ...
    <div className="rise mt-3 card-soft space-y-3 p-4">
      ...
      <label className="block">
        ...
        <input
          ...
          className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-2 text-[14.5px] outline-none focus:border-primary"
        />
      </label>
      <button
        type="button"
      ...
      {busy === "read" && <p className="text-[14.5px] text-muted-foreground">Reading the list…</p>}
      {looking && (
        <p className="text-[14.5px] text-muted-foreground">
              ...
              {!row.skip && (
                <div className="mt-2 space-y-2">
                  <input
                    ...
                    className="w-full rounded-lg border border-border bg-card px-2.5 py-2 text-[14.5px] font-medium outline-none focus:border-primary"
                  />
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      value={row.city ?? ""}
                      ...
                      placeholder="City"
                      className="rounded-lg border border-border bg-card px-2.5 py-2 text-[14.5px] outline-none focus:border-primary"
                    />
                    <select
                      value={row.category ?? "Place"}
                      onChange={(e) => patch(i, { category: e.target.value })}
                      aria-label={`Category for ${row.originalName || `suggestion ${i + 1}`}`}
                      className="rounded-lg border border-border bg-card px-2.5 py-2 text-[14.5px] outline-none"
                    >
                  ...
                  <textarea
                    ...
                    className="w-full rounded-lg border border-border bg-card px-2.5 py-2 text-[14.5px] outline-none focus:border-primary"
                  />
                  <button
                    type="button"
                    onClick={() => void lookupRow(i)}
                    disabled={looking || row.originalName.trim().length < 2}
                    className="text-[13px] font-medium text-primary underline disabled:opacity-50"
                  >
                    Look up this name
                  </button>
                </div>
                ...
                <div className="mt-2 space-y-1.5">
                  {row.hits.map((hit, h) => {
                    ...
                    return (
                      ...
                      >
                        <p className="text-[14.5px] font-medium">{line.title}</p>
                        {line.subtitle ? (
