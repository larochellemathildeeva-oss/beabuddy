...
await flow("optimize: estimated travel times shown, days planned around opening hours", async (page) => {
  await page.getByRole("button", { name: /Plan with Béa/ }).click();
  ...
  for (const text of ["Getting between stops: about 2 h 10 min on foot → about 1 h 15 min.", "2 days were ordered"]) {
